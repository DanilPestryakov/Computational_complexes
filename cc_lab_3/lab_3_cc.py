...
def tolsolvty(arr_xy, infA, supA, infb, supb, *varargin):
    #
    ...
    ################################################################################
    [f_z, g_z, tt_z] = calcfg(arr_xy)

    return [tolmax, argmax, envs, ccode, f_z]

    ################################################################################

if __name__ == '__main__':
    infA = array([[11, 13], [14, 14], [21, 12]])
    supA = array([[15, 17], [18, 18], [25, 16]])
    mi = size(infA, 0)
    ni = size(infA, 1)
    ms = size(supA, 0)
    ns = size(supA, 1)
    print(mi, ms)
    print(ni, ns)
    infb = array([[7.2], [9.2], [10.9]])
    supb = array([[12.2], [13.2], [15.9]])
    x = arange(-0.5, 0.5, 0.01)
    y = arange(-0.5, 0.5, 0.01)
    sh = x.shape[0]
    z = zeros((sh, sh))
    #[tolmax, argmax, envs, ccode] = tolsolvty(infA, supA, infb, supb, 1)
    for i in range(sh):
        for j in range(sh):
            arr = array([[x[i]], [y[j]]])
            [tolmax, argmax, envs, ccode, z[i, j]] = tolsolvty(arr, infA, supA, infb, supb, 0)
